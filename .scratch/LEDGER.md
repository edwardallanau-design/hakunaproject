...
- **The dungeon rotation is derived at request time, not stored.** `design/README.md` asked for `mythicPlusDungeons` fields "via a committed migration … and derive from Raider.IO in the sync"; it shipped as request-time fetches instead — seven at first, **21 since the marquee** (1 bulk + 20 character profiles). **This is now the section's biggest open risk, not a tidiness item.** A cold render blocks on 89 upstream requests for roughly **4s** (the full 165 measured 7.1s; the 2000 io floor halved the poll), and because upstream's edge expires at 300s against our 900s revalidate, the render that repopulates the cache is essentially always cold — so roughly every fifteen minutes one visitor waits four seconds for a decorative section. Worse, Next dedupes fetches *within* a render, not *across* concurrent ones: several visitors arriving together on an expired cache each start their own 165-request poll. Two fixes, neither built: stream the section behind `Suspense` so the page stops waiting on it (small, and honest about the section already being decoration over live data), or persist the rotation in the hourly sync (the real fix, which also closes the archived-Season gap). It degrades safely, but it makes the section depend on a live host at render, and it is **why an archived Season renders no dungeon section at all** — Raider.IO answers only about the current M+ season, so refetching an archive would show today's dungeons under a past season's heading (ADR `0005` names that as the rejected alternative). Persisting at sync time is the real fix, and it would now also *compound*: runs captured while inside a character's ten-run window would be kept after they scroll out, building a guild run log Raider.IO itself does not expose. The `pool` select that would have become another pg enum is gone with the MIDNIGHT/LEGACY badges.
